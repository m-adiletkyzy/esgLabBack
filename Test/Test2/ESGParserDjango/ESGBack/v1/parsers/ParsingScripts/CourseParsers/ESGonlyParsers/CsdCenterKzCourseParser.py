...
class CsdCenterKzCourse(CourseClass):
    ...
    def checkActivity0(self):
        headers = {'User-Agent': 'My User Agent 1.0', }
        raw = requests.get(self.url, headers=headers)
        soup = BeautifulSoup(raw.text, 'html.parser').find('div', {'class': 'registration-wrapper'})

        return soup.text.strip() != 'Регистрация завершена'
...
def Parse_CsdCenterKzRuCourses():
    ...
    course_list = [c for c in course_list if c.checkActivity0()]
    return course_list
